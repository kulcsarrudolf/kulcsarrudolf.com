...
- `src/routes/sitemap[.]xml.ts`, `src/routes/rss[.]xml.ts` and `src/routes/llms[.]txt.ts` serve the sitemap, the RSS feed and `llms.txt`, built by `src/server/sitemap.ts`, `src/server/rss.ts` and `src/server/llms.ts`.
- `scripts/preview-images.ts` renders the social preview cards (the `og:image` behind a shared link) into `public/og/`: one per post and a site-wide default.
  `yarn dev` and `yarn build` run it first, so the folder is generated wherever the site is built and stays out of git.
  `scripts/preview-card.ts` draws one card with satori and resvg, from the same Inter the site uses.
...
- `src/lib/theme.ts` holds the light and dark theme: the stored choice, and the inline script that applies it before the first paint.
  The site opens light every time; the bulb in the navbar switches it, and the choice is kept in `localStorage` like the language.
...
Other scripts: `yarn build` (production build into `.output/`), `yarn start` (serve the built app with Node), `yarn og` (render the social preview cards on their own), `yarn typecheck`, `yarn lint`, `yarn format`, `yarn test`, `yarn deploy` (fast-forward `master` to `develop` and push).
...
A published post is listed in the sitemap, `/rss.xml` and `/llms.txt`, and gets its own preview card on the next build.
